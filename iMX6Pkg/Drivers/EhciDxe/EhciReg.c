  ...
  )
{
  ...
  // to improve low bandwidth platform performance for USB periodic transfers
  // set bit 4 (SDIS) to 1 in nUSBMODE register

  EhcSetOpRegBit(Ehc, EHC_NUSBMODE_OFFSET, NUSBMODE_STREAM_DIS);
  DEBUG ((EFI_D_INFO, "EhcInitHC: low bandwidth platform, SDIS bit=%d\n", (EhcReadOpReg(Ehc, EHC_NUSBMODE_OFFSET) & NUSBMODE_STREAM_DIS)!=0?1:0 ));
