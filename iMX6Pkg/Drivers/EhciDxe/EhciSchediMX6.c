...
//      DEBUG ((EFI_D_INFO, "EhcLinkQhToPeriod: next interval is same Queue head\n"));
      //
  ...
  )
{
  ...
  if (Urb->InProgress) {
    DEBUG ((EFI_D_INFO, "--EhcCheckUrbResult(urb %p is in progress)=false\n",Urb));
    return FALSE;
  }
  ...
  EFI_LIST_FOR_EACH (Entry, &Urb->Qh->Qtds) {
    ...
    if (EHC_BIT_IS_SET (State, QTD_STAT_HALTED)) { // 40h
      DEBUG ((EFI_D_INFO, "EhcCheckUrbResult: urb %p QTD[%d] halted. STA=%08Xh\n",Urb,iQtdCnt,State));
      //
      // EHCI will halt the queue head when met some error.
      // If it is halted, the result of URB is finialized.
      //
      if ((State & QTD_STAT_ERR_MASK) == 0) { // QTD_STAT_TRANS_ERR=8h | QTD_STAT_BABBLE_ERR=10h | QTD_STAT_BUFF_ERR=20h
      ...
      //
      Urb->Result |= EFI_USB_ERR_NOTEXECUTE; // 0x1
//      DEBUG ((EFI_D_INFO, "EhcCheckUrbResult: urb %p QTD[%d] active\n",Urb,iQtdCnt));
      Finished = FALSE;
      goto ON_EXIT;
...
ON_EXIT:
  ...
  DEBUG ((EFI_D_INFO, "--EhcCheckUrbResult(rslt=%Xh) finished=%d\n",Urb->Result,Finished));
  return Finished;
}
  ...
  )
{
  ...
  EFI_LIST_FOR_EACH_SAFE (Entry, Next, &Ehc->AsyncIntTransfers) {
    Urb = EFI_LIST_CONTAINER (Entry, URB, UrbList);
    //DEBUG ((EFI_D_INFO, "+EhcMonitorAsyncRequests have urb[%d]=%p\n", EhciIntAsyncXfersCnt,Urb));
    ...
    if (!Finished)
    {
    // DEBUG ((EFI_D_INFO, "EhcMonitorAsyncRequests urb[%d]=%p did not finish, continue\n", EhciIntAsyncXfersCnt,Urb));
      continue;
    }
    else
    {
      DEBUG ((EFI_D_INFO, "EhcMonitorAsyncRequests urb[%d]=%p finished, mark [in progrs]\n", EhciIntAsyncXfersCnt,Urb));
    }
    ...
    if (Urb->Result == EFI_USB_NOERROR) {
     ...
     // DEBUG ((EFI_D_INFO, "EhcMonitorAsyncRequests result=ok; copy the transferred data\n"));
    ...
    {
      DEBUG ((EFI_D_INFO, "EhcMonitorAsyncRequests urb[%d]=%p result=error %Xh\n", EhciIntAsyncXfersCnt, Urb, Urb->Result));
    }
    ...
    if (Urb->Callback != NULL) {
      DEBUG ((EFI_D_INFO, "EhcMonitorAsyncRequests calling urb[%d]=%p callback with res=%Xh\n", EhciIntAsyncXfersCnt, Urb, Urb->Result));
      //
      ...
      // un-Mark the URB as not in-progress any more for re-use
